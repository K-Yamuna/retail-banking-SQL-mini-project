...
CASE
	...
	ELSE product_price 
    end as Increase_in_price
from product left join product_class using (product_class_code);
...
Select distinct customer_fname,customer_lname from online_customer oc join address a using (address_id)
join order_header oh  using (customer_id) join order_items ot using(order_id)
join product p using (product_id) join product_class using (product_class_code)
where state not in ('Karnataka') and product_class_desc not in ('Toys', 'Books');
